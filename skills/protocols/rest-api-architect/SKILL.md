...
Two patterns cover almost every API. Choose by team / system shape; framework implementation is in the framework architect skill (FastAPI, Gin, etc.).

**A. In-house OAuth2 password + JWT.** For monoliths and single-service deployments. The service both issues tokens (on `POST /v1/auth/login`) and verifies them on every authenticated request.
...
**B. External IdP** (Keycloak / Auth0 / Cognito / Entra). For any system with more than one service, MFA requirements, social login, or compliance demanding SSO.
...
**Authorization is per-endpoint, not middleware-global.** Scopes / roles are checked at the route after authentication, by a dependency or middleware-wrap specific to that endpoint. A blanket "authenticated users can do anything" middleware is the most common source of accidentally-public endpoints.
