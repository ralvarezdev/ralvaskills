...
See [HTML_REPORT.md](HTML_REPORT.md) for the full HTML scaffold, diagram patterns, and styling guidance.
...
Once the design is settled, deepen the module per [DEEPENING.md](DEEPENING.md):

- **Classify the dependencies** into one of four categories — in-process, local-substitutable, remote-but-owned (ports & adapters), true-external (mock). The category dictates the seam and the testing strategy.
- **Apply seam discipline** — only introduce a port when at least two adapters justify it (production + test typically counts). A single-adapter seam is just indirection.
- **Replace, don't layer.** Delete the old shallow-module tests once tests at the deepened module's interface exist. The interface is the test surface.
